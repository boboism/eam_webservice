...
class TransDocF40 < ActiveRecord::Base
  self.table_name = "vw_erp_f40"
  self.primary_key = "id"

  has_many :details, :class_name => "TransDocDetailF40", :foreign_key => "master_doc_id"
end

class TransDocDetailF40 < ActiveRecord::Base
  self.table_name = "vw_erp_detl_f40"
  self.primary_key = "id"

  belongs_to :trans_doc, :class_name => "TransDocF40"
end

client = Savon.client(:wsdl => "http://172.18.81.20/BOI/Service.asmx?WSDL")
ws_attrs = {
  :interface => {:sender => "EAM", :receiver => "ERP", :billtype => "F40"},
  :func_name => "FixedAssetTransfer",
  :handshake => "654321"
}
receivable_docs = []
unreceivable_docs = []
TransDocF40.all.each do |doc|
  token = UUIDTools::UUID.random_create.to_s.gsub("-","").upcase
  parameters = doc.to_erp_xml(ws_attrs.merge({:include => :details, :except => [:id, :master_doc_id, :erp_synchronized]}))
